...
def func2():
    ...
    for name in all_path:  # 打开所有章节找任务
        ...
        with open(path + name, mode='r', encoding='utf-8') as f:
            file = open("people.txt", encoding='utf-8')
            for line in file.readlines():  # readlines获取所有行   line就是每行
                b.append(f.read().count(line.strip()))  # 每一个人物出现的次数
        c.extend(a)
        c.extend(b)
        allInfo.append(c)
    #存表格
...
def func3():
    ...
    count = {}
    # 获取出现最多次数的单词
    for word in strAllList:
        if word in count and word != '':
            count[word] += 1
        else:
            count[word] = 1
        ...
        for num in p2Liist:
            if int(ii)-50 < num & num < int(ii)+50:
                ...
                if p2Index!=p1Index and strAll[p1Index:p2Index + len(p2)] !='':
                    ...
                    cutStr = strAll[p1Index:p2Index + len(p2)].replace(str(p2),'['+str(p2)+']')
                    print(cutStr)

func3()
func2()
find_co_occurrence('鲁力','鲁力')