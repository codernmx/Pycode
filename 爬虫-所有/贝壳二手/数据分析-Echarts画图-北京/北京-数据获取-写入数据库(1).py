...
# 连接数据库
def coon():
    db = pymysql.connect(host="localhost", user="root", password="111111", database="beike", port=3306)
...
def getDetail(singurl):
    ...
    try:
        ...
        cursor = coon() #这两行是避免 数据库链接超时报错
        cursor.execute(sql) #这两行是避免 数据库链接超时报错
        print('写入数据库失败！！请检查')
